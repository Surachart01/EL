          ...
          <div className="bg-[#f8fafc] rounded-[36px] border-4 border-white shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 relative flex flex-col">
            ...
            <div className="space-y-6 flex-1 text-left">
                  ...
                  return (
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                      ...
                      <div className="lg:col-span-7 bg-purple-50/20 p-4 rounded-3xl border border-purple-100 shadow-sm space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-slate-500">🎭 บทบาทสมมติ (Roleplay)</span>
                          <span className={roleplayDone ? 'text-emerald-600' : 'text-slate-400'}>
                            {roleplayDone ? '✅ สำเร็จ' : '❌ ยังไม่สำเร็จ'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-slate-500">🎙️ คลิปการพูด (Submission)</span>
                          <span className={hasSub ? 'text-emerald-600' : 'text-slate-400'}>
                            {hasSub ? '✅ ผ่านเกณฑ์' : '❌ ยังไม่ส่ง/ไม่ผ่าน'}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm space-y-3">
                <h4 className="font-black text-slate-800 text-sm">ประวัติการส่งคลิปเสียงและวิดีโอ (Speaking Submissions)</h4>
                
                {!selectedStudent.progress.submissions || selectedStudent.progress.submissions.length === 0 ? (
                  <p className="text-xs text-slate-400 font-bold text-center py-4 bg-slate-50 rounded-2xl">ยังไม่มีการส่งไฟล์เสียงหรือวิดีโอสำหรับประเมิน</p>
                ) : (
                  <div className="overflow-x-auto rounded-2xl border border-slate-150">
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 text-slate-400 font-black border-b border-slate-150">
                          <th className="p-3">บทเรียน</th>
                          <th className="p-3 text-center">ประเภทสื่อ</th>
                      ...
                      <tbody className="divide-y divide-slate-100 font-bold text-slate-700">
                        {selectedStudent.progress.submissions.map((sub, sIdx) => {
                          const lesson = LESSONS.find(l => l.id === sub.lessonId);
                          return (
                            <tr key={sIdx} className="hover:bg-slate-50/50">
                              <td className="p-3">{lesson ? `บทที่ ${lesson.id}: ${lesson.title}` : `บทเรียนที่ ${sub.lessonId}`}</td>
                              <td className="p-3 text-center text-slate-500">{sub.mediaType === 'audio' ? '🎙️ เสียง' : '📹 วิดีโอ'}</td>
                              <td className="p-3 text-center text-slate-400">{sub.date}</td>
                              <td className="p-3 text-center">
                                <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-[10px] font-black border border-emerald-200">
                                  ตรวจแล้ว
                                </span>
                              </td>
                              <td className="p-3 text-right text-indigo-650 font-black text-sm">{sub.score}%</td>
                            </tr>
                          );
                        })}
                      </tbody>
